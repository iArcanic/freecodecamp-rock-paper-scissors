...
def player(prev_play, opponent_history=[]):
    ...
    most_frequent_move = max(move_count, key=move_count.get, default='R')
    ...
    predicted_move = max(possible_moves, key=possible_moves.get, default='R')
    ...
    if len(opponent_history) > 10:
        ...
        if markov_performance > 0.5:
            player.strategy = "markov"
        elif freq_performance > 0.5:
            player.strategy = "frequency"
    
    # Specific handling for Abbey
    if len(opponent_history) >= 2:
        last_two_moves = ''.join(opponent_history[-2:])
        abbey_predictions = {'RR': 'P', 'RP': 'S', 'RS': 'P',
                             'PR': 'S', 'PP': 'R', 'PS': 'R',
                             'SR': 'P', 'SP': 'S', 'SS': 'R'}
        if last_two_moves in abbey_predictions:
            abbey_move = abbey_predictions[last_two_moves]
            # Adjusting for Abbey without making it the only strategy
            if player.strategy == "frequency":
                return freq_move
            else:
                return abbey_move if player.last_moves == '' or player.last_moves == prev_play else markov_move
